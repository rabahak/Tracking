...
//---------------------TEST: Measure of total time (xprojections)-----------------
  ...
  m_timerTool->start( m_timeXProjeTotal_OLD);
  for( unsigned int part= 0; 2 > part; ++part ){
  for(unsigned int icase = 0; m_nCases > icase ; ++icase){
    m_xCandidates[(int)part].clear(); //x candidates up cleaned every Case!
    findXProjections_OLD(part,icase);
  }}
  m_timerTool->stop( m_timeXProjeTotal_OLD);
...
if(UNLIKELY(m_debug)){ debug()<<"Hits in all Zones Loaded"<<endmsg;}
  ...
  std::vector<PrHit>::iterator itMBeg_prev;
...
//loop 3-----------------------------------------------FOR EACH MIDDLE LAYER----------------------------------------
	...
	/*	
	//same as before
	if(Lbound==Lbound_prev) // that means the first hit in the middle layer which is the lowest (Lbound)
      {
...
//===============================================================================
//===============================================================================
void PrHybridSeeding::findXProjections_OLD(unsigned int part, unsigned int iCase){
  PrHits parabolaSeedHits; // List of hits in both x-layers in T2
  parabolaSeedHits.reserve(12);  
  std::vector<PrHits> xHitsLists; //vector of list of Hits
  xHitsLists.reserve(100);
