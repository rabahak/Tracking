...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "PrKernel" = ""; then \
	    if test -d ../PrKernel; then \
	      echo "Installing files from more ../PrKernel to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/PrKernel; then mkdir -p $(install_include_dir)/PrKernel; fi;\
	        $(install_command) ../PrKernel $(install_include_dir);\
	    else \
	      echo "No more PrKernel include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/PrKernel"; \
	  $(uninstall_command) "$(install_include_dir)/PrKernel/*" ; \
	  if test -f $(install_include_dir)/PrKernel.cmtref ; then \
	    echo "Removing $(install_include_dir)/PrKernel.cmtref"; \
	    eval rm -f $(install_include_dir)/PrKernel.cmtref ; \
	  fi; \
	fi
