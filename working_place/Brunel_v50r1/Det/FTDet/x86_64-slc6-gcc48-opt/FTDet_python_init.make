...
tags      = $(tag),$(CMTEXTRATAGS),target_FTDet_python_init
...
include $(cmt_local_tagfile_FTDet_python_init)
#-include $(cmt_local_tagfile_FTDet_python_init)
...
help ::
	$(echo) 'FTDet_python_init'

binobj = 
ifdef STRUCTURED_OUTPUT
binobj = FTDet_python_init/
#FTDet_python_init::
...
install_python_dir = $(installarea)/python/FTDet
init_file = $(install_python_dir)/__init__.py


FTDet_python_init :: FTDet_python_initinstall

install :: FTDet_python_initinstall

FTDet_python_initinstall :: $(init_file)
...
FTDet_python_initclean :: FTDet_python_inituninstall

uninstall :: FTDet_python_inituninstall

FTDet_python_inituninstall ::
...
ifndef PEDANTIC
.DEFAULT::
	$(echo) "(FTDet_python_init.make) $@: No rule for such target" >&2
#	@echo "#CMT> Warning: $@: No rule for such target" >&2; exit
	if echo $@ | grep 'FTDetsetup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(FTDet_python_init.make): $@: File no longer generated" >&2; exit 0; fi
else
.DEFAULT::
	$(echo) "(FTDet_python_init.make) PEDANTIC: $@: No rule for such target" >&2
	if echo $@ | grep 'FTDetsetup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(FTDet_python_init.make): $@: File no longer generated" >&2; exit 0;\
	 elif test $@ = "$(cmt_final_setup)" -o\
	 $@ = "$(cmt_final_setup_FTDet_python_init)" ; then\
	 found=n; for s in 1 2 3 4 5; do\
	 sleep $$s; test ! -f $@ || { found=y; break; }\
	 done; if test $$found = n; then\
	 test -z "$(cmtmsg)" ||\
	 echo "$(CMTMSGPREFIX)" "(FTDet_python_init.make) PEDANTIC: $@: Seems to be missing. Ignore it for now" >&2; exit 0 ; fi;\
	 elif test `expr $@ : '.*/'` -ne 0 ; then\
	 test -z "$(cmtmsg)" ||\
	 echo "$(CMTMSGPREFIX)" "(FTDet_python_init.make) PEDANTIC: $@: Seems to be a missing file. Please check" >&2; exit 2 ; \
	 else\
	 test -z "$(cmtmsg)" ||\
	 echo "$(CMTMSGPREFIX)" "(FTDet_python_init.make) PEDANTIC: $@: Seems to be a fake target due to some pattern. Just ignore it" >&2 ; exit 0; fi
endif

FTDet_python_initclean ::
