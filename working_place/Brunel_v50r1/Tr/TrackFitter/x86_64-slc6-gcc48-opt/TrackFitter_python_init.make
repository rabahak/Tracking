...
#====================================
#  Document TrackFitter_python_init
#
...
tags      = $(tag),$(CMTEXTRATAGS),target_TrackFitter_python_init
...
include $(cmt_local_tagfile_TrackFitter_python_init)
#-include $(cmt_local_tagfile_TrackFitter_python_init)
...
help ::
	$(echo) 'TrackFitter_python_init'

binobj = 
ifdef STRUCTURED_OUTPUT
binobj = TrackFitter_python_init/
#TrackFitter_python_init::
...
install_python_dir = $(installarea)/python/TrackFitter
init_file = $(install_python_dir)/__init__.py


TrackFitter_python_init :: TrackFitter_python_initinstall

install :: TrackFitter_python_initinstall

TrackFitter_python_initinstall :: $(init_file)
...
TrackFitter_python_initclean :: TrackFitter_python_inituninstall

uninstall :: TrackFitter_python_inituninstall

TrackFitter_python_inituninstall ::
...
ifndef PEDANTIC
.DEFAULT::
	$(echo) "(TrackFitter_python_init.make) $@: No rule for such target" >&2
#	@echo "#CMT> Warning: $@: No rule for such target" >&2; exit
	if echo $@ | grep 'TrackFittersetup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(TrackFitter_python_init.make): $@: File no longer generated" >&2; exit 0; fi
else
.DEFAULT::
	$(echo) "(TrackFitter_python_init.make) PEDANTIC: $@: No rule for such target" >&2
	if echo $@ | grep 'TrackFittersetup\.make$$' >/dev/null; then\
	 echo "$(CMTMSGPREFIX)" "(TrackFitter_python_init.make): $@: File no longer generated" >&2; exit 0;\
	 elif test $@ = "$(cmt_final_setup)" -o\
	 $@ = "$(cmt_final_setup_TrackFitter_python_init)" ; then\
	 found=n; for s in 1 2 3 4 5; do\
	 sleep $$s; test ! -f $@ || { found=y; break; }\
	 done; if test $$found = n; then\
	 test -z "$(cmtmsg)" ||\
	 echo "$(CMTMSGPREFIX)" "(TrackFitter_python_init.make) PEDANTIC: $@: Seems to be missing. Ignore it for now" >&2; exit 0 ; fi;\
	 elif test `expr $@ : '.*/'` -ne 0 ; then\
	 test -z "$(cmtmsg)" ||\
	 echo "$(CMTMSGPREFIX)" "(TrackFitter_python_init.make) PEDANTIC: $@: Seems to be a missing file. Please check" >&2; exit 2 ; \
	 else\
	 test -z "$(cmtmsg)" ||\
	 echo "$(CMTMSGPREFIX)" "(TrackFitter_python_init.make) PEDANTIC: $@: Seems to be a fake target due to some pattern. Just ignore it" >&2 ; exit 0; fi
endif

TrackFitter_python_initclean ::
