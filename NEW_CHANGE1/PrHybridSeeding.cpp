...
void PrHybridSeeding::findXProjections(unsigned int part, unsigned int iCase){
    ...
    //Define typedef again here? ??? ??
    typedef PrHits::iterator HitIter;
    typedef std::array<HitIter,NLAYERS> HitIters;
    ...
    {//Always: first=0 last=1 middles=2,3 remains=4,5 (order of loops)
        m_hitManager->InitializeBB(Bounds[i]);
        Last[i]=Bounds[i].second;
    }
    ...
    for(; Bounds[0].first != Last[0]; Bounds[0].first++) //for a hit in first layer
    {
        ...
        float minL = xproj - m_TolFirstLast[iCase];
        m_hitManager->UpdateB1B2(Bounds[1],min,max,zones[1]->number());
        for(int i=2;i<6;i++)
        {
            float max=computeX(Bounds[0].first->x(),Bounds[0].first->z(),maxL,zones[1]->number(),zones[i]->number());
            float min=computeX(Bounds[0].first->x(),Bounds[0].first->z(),minL,zones[1]->number(),zones[i]->number());
        m_hitManager->UpdateB1B2(Bounds[i],min,max); // opens a triangular window with firstLayerHit as its summit and dist(min,max)LastLayer as its base
        }
        
        if(m_hitManager->EnoughLayersFired(Bounds)==false)
            continue;
        ...
        {        //                  1 refers to a dynamic bound (hit) here (in its corresponding loop)
            ...
            for(int i=1;i<6;i++)
            {
                float xproj=computeX(Bounds[0].first->x(),zones[0]->z(),Bounds[1].first->x(),zones[1]->z(), zones[i]->z());
                float max=xproj+10;
                float min=xproj-10;
                m_hitManager->UpdateB1B2(Bounds[i],min,max); //opens a window of 20mm around the 2-hit combination line
            }
            if(m_hitManager->EnoughLayersFired(Bounds)==false)
                continue;
            ...
            //=====================TO COMPRESS FROM HERE
                //=====================TO COMPRESS FROM HERE
                    //=====================TO COMPRESS FROM HERE
                        //=====================TO COMPRESS FROM HERE
            for(int i=2;i<4;i++) { //for hits in middle layers
                ...
                m_hitManager->updateB1(Bounds[i],xMin,xMax);
            }
            
            if( Bounds[i].first == Bounds[i].second) continue;
            if(m_debug )debug()<<"Will Loop over xZones Hits"<<endmsg;
            for( ; Bounds[i].first!= Bounds[i].second; ++Bounds[i].first){   //--------------------------------------------------------------------FOR
                if( (*Bounds[i].first).x() > xMax ) break;
                if( (*Bounds[i].first).isUsed() && m_removeFlagged) continue; //Not re use Hits in the middle
            ...
            for(unsigned int i = 0; i<maxParabolaSeedHits;++i){
                ...
                if(UNLIKELY(m_debug)) debug()<<"Loop on xZones   : size = "<<zones.size()<<endmsg;
                for(unsigned int i=2;i<6;i++){
                    ...
                    m_hitManager->UpdateB1(Bounds[i],xMinAtZ,xMaxAtZ);
