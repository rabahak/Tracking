...
void PrHybridSeeding::CollectUV(  PrHybridSeedTrack& xProje,const std::vector<PrHitZone*>& uvZones, PrHits& myStereo){
    ...
    for( PrHitZone* zoneUV: uvZones){
        ...
        // if(UNLIKELY(m_debug)){   if( yMax <yMin) error()<<"Stereo Problem"<<endmsg;}
        if(part==0){
            yMin = m_yMin;
            if(kk%2==1 && m_useFix){
                yMin = +m_yMin_TrFix;
        ...
        for( ; itEnd!= itH; ++itH ){
            ...
            // = To be implemented ? Cut on ty here for hits!
...
inline void PrHybridSeeding::LookAroundMin( IterPairs& bound, float& oMin,float& nMin,  HitIterat& eZone ,HitIterat& bZone){
    if( nMin < oMin){
        bound.begin  = m_hitManager->get_lowerBound_lin_reverse( bZone, bound.begin, nMin);
    }else{
        bound.begin  = m_hitManager->get_lowerBound_lin( bound.begin, eZone, nMin);
    }
    oMin = nMin;
}
inline HitIterat PrHybridSeeding::LookAroundMin_it( HitIterat& currBeg, float& oMin,float& nMin,  HitIterat& eZone ,HitIterat& bZone){
    if( nMin < oMin){
        return m_hitManager->get_lowerBound_lin_reverse( bZone, currBeg, nMin);
    }else{
        return m_hitManager->get_lowerBound_lin( currBeg, eZone, nMin);
    }
    // oMin = nMin;
}
...
inline void PrHybridSeeding::LookAroundMax( IterPairs& bound, float& oMax,float& nMax,  HitIterat& eZone ,HitIterat& bZone){
    if( nMax > oMax){
        bound.end = m_hitManager->get_upperBound_lin( bound.end, eZone, nMax);
    }else{
        bound.end = m_hitManager->get_upperBound_lin_reverse(  bZone , bound.end, nMax);
    }
    oMax = nMax;
}
inline HitIterat PrHybridSeeding::LookAroundMax_it( HitIterat& currEnd, float& oMax,float& nMax,  HitIterat& eZone ,HitIterat& bZone){
    if( nMax > oMax){
        return m_hitManager->get_upperBound_lin( currEnd, eZone, nMax);
    }else{
        return m_hitManager->get_upperBound_lin_reverse(  bZone , currEnd, nMax);
    }
    // oMax = nMax;
}
...
void PrHybridSeeding::findXProjections(unsigned int part, unsigned int iCase){
    ...
    std::array<float, NLAYERS> number;
    std::array<float, NLAYERS> zLays;
    if(0 == iCase){
    ...
    zones[0]=m_zones[firstZoneId];     zLays[0]=m_zones[firstZoneId]->z();   number[0]=m_zones[firstZoneId]->number();
    zones[1]=m_zones[lastZoneId];      zLays[1]=m_zones[lastZoneId]->z();    number[1]=m_zones[lastZoneId]->number();
    // if(UNLIKELY(m_debug)){ debug()<<"Hits in last and first Zone Loaded"<<endmsg;}
    zones[2]=m_zones[s_T2X1|part];    zLays[2] = m_zones[s_T2X1|part]->z();  number[2]=m_zones[s_T2X1|part]->number();
    zones[3]=m_zones[s_T2X2|part];    zLays[3] = m_zones[s_T2X2|part]->z();  number[3]=m_zones[s_T2X2|part]->number();
    unsigned int i=4;
    for(unsigned int xZoneId : {s_T1X1, s_T1X2, s_T3X1, s_T3X2}){
        xZoneId =xZoneId | part;
        if(xZoneId != firstZoneId && xZoneId != lastZoneId){
            zones[i]=m_zones[xZoneId];   zLays[i] = m_zones[xZoneId]->z();   number[i]=m_zones[xZoneId]->number();
            i++;
        }
    }
    float invZfirst = 1./zLays[0];// zones[0]->z();
    ...
    float zT2 = 0.5*(zLays[2]+zLays[3]);
    Boundaries Bounds;
    //============Initialization
    std::array< HitIterat, 6> endsZone;
    std::array< HitIterat, 6> begZone;
    for(int i=0;i<NLAYERS;i++){
        //Always: first=0 last=1 middles=2,3 remains=4,5 (order of loops)
        m_hitManager->InitializeBB(Bounds[i],number[i]);
        begZone[i]  = m_hitManager->getIterator_Begin( number[i]);
        endsZone[i] = m_hitManager->getIterator_End( number[i]);
    }
    //===FIRST LAYER LOOP===
    PrHit *fHit;
    std::array<bool,6> first = {true, true, true,true, true, true};
    std::array<float,6> xminPrev, xmaxPrev;
    for(; Bounds[0].begin != Bounds[0].end; ++Bounds[0].begin) //for a hit in first layer
    {
        ...
        if( first[1]){
            //they get initialized to begin and end!
            Bounds[1].end    = m_hitManager->get_lowerBound_log( Bounds[1].begin, Bounds[1].end , maxXl);
            Bounds[1].begin  = m_hitManager->get_lowerBound_log( Bounds[1].begin, Bounds[1].end, minXl);
            first[1] = false;
            xminPrev[1] = minXl;
            xmaxPrev[1] = maxXl;
        }else{
            LookAroundMin( Bounds[1], xminPrev[1], minXl, endsZone[1], begZone[1]);
            LookAroundMax( Bounds[1], xmaxPrev[1], maxXl, endsZone[1], begZone[1]);
        }
        PrHit* lHit;
        ...
        for(auto Lhit=Bounds[1].begin; Lhit != Bounds[1].end; ++Lhit){
            ...
            //Define the boundaries in T2 to look for parabolaSeedHits ( push them back now or later? )
            ...
            for( int i = 2; i<4; ++i){
                ...
                if( xMin > xMax){
                    // std::cout<<"min max middle layer wrong!!!"<<std::endl;
                    std::swap(xMin, xMax);
                }
                if( first[i] ){
                    ...
                    LookAroundMin( Bounds[i], xminPrev[i], xMin, endsZone[i], begZone[i] );
                    LookAroundMax( Bounds[i], xmaxPrev[i], xMax, endsZone[i], begZone[i] );
                }
                xminPrev[i] = xMin;
                xmaxPrev[i] = xMax;
                if( xMin < xMinT2){   xMinT2 = xMin;  }
                if (xMax > xMaxT2){   xMaxT2 = xMax;  }
                //nfired+=( Bounds[i].end - Bounds[i].begin  >0 );//at max you reach 4 here
                //you pick up hits in T2 here !
                bool fired = false;
                if( Bounds[i].end - Bounds[i].begin > 0){
                    for( auto itM = Bounds[i].begin ; itM!= Bounds[i].end; ++itM){
                        if( (*itM).isUsed() && m_removeFlagged) continue;
                        // std::cout<<"push back parabola seed hit"<<std::endl;
            ...
            if( nfired < 3 ){
                //still with 2 hits (last/first)
                // std::cout<<"Not enough fired layers"<<nfired<<std::endl;
                // std::cout<<"Par seed hits size"<<parabolaSeedHits.size();
                continue;
            }
            
            //   ----------------------------    MESSAGE -----------------   //
            //This is needed to find the tolerance in remaining layers , but you can think of drop this and do not do the check!!( be worried of the windows!!!
            //Could affect timing: do not count fired => skip the Bounds[i] update
            //Do rather than a parabolic tolerance a flat tolerance based on the slope ( less computations , but maybe not efficient for counting nfired !)
            //Remove it anyway and move the first[i] part in the nested loop when processing parabolaSeedHits! ( i think this is the way you implemented the first version //
            //
            for( int i =4; i<6; ++i){
                xMin = ParabolicTol( fHit, lHit, zLays[i], zT2, xMinT2)   ;
                xMax = ParabolicTol( fHit, lHit, zLays[i], zT2, xMaxT2)   ;
                if( xMin > xMax){ std::swap( xMin, xMax); }
                xMin-=m_tolRemaining[iCase];
                xMax+=m_tolRemaining[iCase];
                if( first[i] ){
                    Bounds[i].begin = m_hitManager->get_lowerBound_log( Bounds[i].begin, Bounds[i].end, xMin);
                    Bounds[i].end = m_hitManager->get_upperBound_log( Bounds[i].begin, Bounds[i].end, xMax);
                    first[i] = false;
                }else{
                    LookAroundMin( Bounds[i], xminPrev[i], xMin , endsZone[i], begZone[i]);
                    LookAroundMax( Bounds[i], xmaxPrev[i], xMax , endsZone[i], begZone[i]);
                }
                nfired+= ( Bounds[i].end - Bounds[i].begin >0 );
                xminPrev[i] = xMin;
                xmaxPrev[i] = xMax;
            }
            if( nfired <5){
            ...
            for(unsigned int i = 0; i<maxParabolaSeedHits;++i){ //for a give hit in 1st layer, and a given hit in last layer
                ...
                float bestDist;
                // nfired = 3;
                // bool AroundParabolaMiss; // at this point if you miss 2 layers you are fucked (We enter the fit with 5 hits by construction )
                HitIterat itH;
                HitIterat itE;
                xHits.clear();
                for(int j =2 ;j< NLAYERS;++j){ //look in zones except the 1st and last zones
                    ...
                    //if(UNLIKELY((m_debug)) debug()<<"Get iterator lower bound "<<xMinAtZ<<endmsg; //parabola here is larger than tolerances in x0! ( 0.75 mm up / down when x0 ~0 )
                    //IF THIS IS OK, then nfired can be used before ( to be checked performance like that or the other way around , the lookaround!)
                    ...
                    //if worried of parabola in T2 being too small move to these methods here
                    //auto itH = LookAroundMin_it( Bounds[j].begin, xminPrev[j], xMinAtZ, endsZone[j], begZone[j]);
            ...
            //if(UNLIKELY(m_debug)) debug()<<"End Loop For pick up Parabola Hits and build the xHitsLists"<<endmsg;
...
float PrHybridSeeding::computeX(float x0,float z0,float x1,float z1,   float z2)
{   //x=az+b
    float a=(x1-x0)/(z1-z0);
    float b=x1-a*z1;
    return a*z2+b;
